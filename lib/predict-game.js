...
// Returns parsed JSON on success, null on 404, throws on network/5xx errors.
async function mlbGet(path) {
  const url = `${MLB_BASE}${path}`;
  console.log(`[mlb] GET ${url}`);
  let res;
  try {
    res = await fetch(url);
  } catch (err) {
    console.error(`[mlb] network error — ${url} — ${err.message}`);
    throw err;
  }
  if (res.status === 404) {
    console.warn(`[mlb] 404 (skipping) — ${url}`);
    return null;
  }
  if (!res.ok) {
    console.error(`[mlb] ${res.status} ${res.statusText} — ${url}`);
    throw new Error(`MLB API ${res.status} ${res.statusText} — ${url}`);
  }
  console.log(`[mlb] ${res.status} OK — ${url}`);
  return res.json();
}
...
export function fetchRecentSchedule(teamId) {
  ...
  return mlbGet(
    `/schedule?teamId=${teamId}&season=${year}&gameType=R&sportId=1` +
    `&startDate=${year}-03-01&endDate=${today}`
  );
}

// Single request for both hitting and pitching — uses stats=season which the API requires.
function fetchTeamStats(teamId) {
  const year = new Date().getFullYear();
  return mlbGet(
    `/teams/${teamId}/stats?stats=season&season=${year}&group=hitting,pitching&gameType=R&sportId=1`
  );
}
...
function fetchProbablePitchers(teamId) {
  const year = new Date().getFullYear();
  return mlbGet(
    `/schedule?teamId=${teamId}&season=${year}&gameType=R&sportId=1&hydrate=probablePitcher`
  );
}
...
// Extractors for the combined hitting+pitching response from fetchTeamStats.
// The API returns stats as an array with a group.displayName field per entry.
function extractHittingStats(statsData) {
  const group = statsData?.stats?.find((s) => s.group?.displayName === 'hitting');
  return group?.splits?.[0]?.stat ?? {};
}

function extractPitchingStats(statsData) {
  const group = statsData?.stats?.find((s) => s.group?.displayName === 'pitching');
  return group?.splits?.[0]?.stat ?? {};
}
...
async function runStage(client, send, stage, params) {
  console.log(`[claude] stage ${stage} — model=${params.model} max_tokens=${params.max_tokens}`);
  let fullText = '';
  try {
    ...
    console.error(`[claude] stage ${stage} FAILED — ${err.message}`);
    console.error(err.stack ?? '(no stack)');
    throw err;
  }
  console.log(`[claude] stage ${stage} done — ${fullText.length} chars`);
  return fullText;
}
...
export async function predictGame({ team1Id, team1Name, team2Id, team2Name, send = () => {} }) {
  const year = new Date().getFullYear();
  let step = 'phase1-mlb-fetches';
  try {

  // Phase 1: 14 parallel fetches — allSettled so every failure is logged, not just the first.
  // fetchTeamStats covers hitting+pitching in one combined request (stats=season&group=hitting,pitching).
  const phase1Labels = [
    `recentSchedule(${team1Id})`, `teamStats(${team1Id})`, `probablePitchers(${team1Id})`,
    `recentSchedule(${team2Id})`, `teamStats(${team2Id})`, `probablePitchers(${team2Id})`,
    'standings',
    `roster(${team1Id})`, `roster(${team2Id})`,
    `injuryRoster(${team1Id})`, `injuryRoster(${team2Id})`,
    `seasonSchedule(${team1Id},${year})`, `seasonSchedule(${team1Id},${year - 1})`, `seasonSchedule(${team1Id},${year - 2})`,
  ];
  ...
  // Real errors (network/5xx) → throw immediately.
  // Null values (404) → log as unavailable data and continue gracefully.
  const unavailableData = [];
  const phase1Values = phase1Results.map((r, i) => {
    if (r.status === 'rejected') {
      console.error(`[phase1] FAILED ${phase1Labels[i]}: ${r.reason?.message ?? r.reason}`);
      throw r.reason;
    }
    if (r.value === null) {
      unavailableData.push(phase1Labels[i]);
      console.warn(`[phase1] null (404 skipped) — ${phase1Labels[i]}`);
    }
    return r.value;
  });
  ...
  const team1Hitting  = extractHittingStats(stats1);
  const team1Pitching = extractPitchingStats(stats1);
  ...
  const team2Hitting  = extractHittingStats(stats2);
  const team2Pitching = extractPitchingStats(stats2);
  ...
  const missingNote = unavailableData.length
    ? ` NOTE: The following data endpoints returned 404 and are absent from the stats below: ${unavailableData.join(', ')}. Make your prediction based on whatever data is available and acknowledge the gaps where relevant.`
    : '';
  ...
  const groundTruthInstruction =
    ...
    `Do not rely on your training data for any statistics or recent results.${missingNote}`;
  ...
  return { verdict, nextGame };
    ...
    // Attach the step label so callers know exactly where it broke
    err.step = step;
    throw err;
  }
}
