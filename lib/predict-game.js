...
export async function predictGame({ team1Id, team1Name, team2Id, team2Name, send = () => {} }) {
  ...
  // Phase 1: 16 parallel fetches — allSettled so every failure is logged, not just the first
  const phase1Labels = [
    `recentSchedule(${team1Id})`, `hitting(${team1Id})`, `pitching(${team1Id})`, `probablePitchers(${team1Id})`,
    `recentSchedule(${team2Id})`, `hitting(${team2Id})`, `pitching(${team2Id})`, `probablePitchers(${team2Id})`,
    'standings',
  ...
  // Log every failure, then throw on the first one
  const phase1Failures = phase1Results
    .map((r, i) => r.status === 'rejected' ? { label: phase1Labels[i], reason: r.reason } : null)
    .filter(Boolean);
  if (phase1Failures.length) {
    phase1Failures.forEach(({ label, reason }) =>
      console.error(`[phase1] FAILED ${label}: ${reason?.message ?? reason}`)
    );
    throw phase1Failures[0].reason;
  }
